...
#define EXP_WIDTH		(45)	// 爆発の幅 / 2 (横)
#define EXP_HEIGHT		(45)	// 爆発の幅 / 2 (高さ)
#define ANIM_TIME		(6)		// 爆発のアニメーション待機時間
#define ANIM_MAX		(9)		// 爆発の最大アニメーション
...
{
	"data\\TEXTURE\\explosion000.png",	// 爆発のテクスチャ相対パス
};
...
void UpdateExplosion(void)
{
	...
	{ // 爆発の最大表示数分繰り返す
		...
		{ // 爆発が使用されている場合
			...
			// テクスチャ座標の設定
			pVtx[0].tex = D3DXVECTOR2( g_aExplosion[nCntExplosion].nPatternAnim      * 0.1f, 0.0f);
			pVtx[1].tex = D3DXVECTOR2((g_aExplosion[nCntExplosion].nPatternAnim + 1) * 0.1f, 0.0f);
			pVtx[2].tex = D3DXVECTOR2( g_aExplosion[nCntExplosion].nPatternAnim      * 0.1f, 1.0f);
			pVtx[3].tex = D3DXVECTOR2((g_aExplosion[nCntExplosion].nPatternAnim + 1) * 0.1f, 1.0f);
		}
